...
/**
 * Read an Expo public environment variable with a fallback.
 */
function env(key: string, fallback: string = ""): string {
  const fullKey = `EXPO_PUBLIC_${key}`;
  return (
    (process.env as Record<string, string | undefined>)[fullKey] ?? fallback
  );
}
 ...
 * - EXPO_PUBLIC_REDIS_DEFAULT_CONNECTION: Default connection name (default: 'main')
 * - EXPO_PUBLIC_REDIS_GLOBAL: Register globally (default: true)
 * - EXPO_PUBLIC_UPSTASH_REDIS_REST_URL: Primary Upstash REST URL
 * - EXPO_PUBLIC_UPSTASH_REDIS_REST_TOKEN: Primary Upstash REST token
 * - EXPO_PUBLIC_UPSTASH_CACHE_REST_URL: Cache-specific Upstash REST URL (falls back to primary)
 * - EXPO_PUBLIC_UPSTASH_CACHE_REST_TOKEN: Cache-specific Upstash REST token (falls back to primary)
 * - EXPO_PUBLIC_UPSTASH_SESSION_REST_URL: Session-specific Upstash REST URL (falls back to primary)
 * - EXPO_PUBLIC_UPSTASH_SESSION_REST_TOKEN: Session-specific Upstash REST token (falls back to primary)
 */
const redisConfig = defineConfig({
  ...
  */
  ...
  |
  */
  default: env("EXPO_PUBLIC_REDIS_DEFAULT_CONNECTION", "main"),
  ...
  */
  connections: {
     ...
     */
    main: {
      url: env("EXPO_PUBLIC_UPSTASH_REDIS_REST_URL", ""),
      token: env("EXPO_PUBLIC_UPSTASH_REDIS_REST_TOKEN", ""),
    },
    ...
    cache: {
      url: env(
        "EXPO_PUBLIC_UPSTASH_CACHE_REST_URL",
        "EXPO_PUBLIC_UPSTASH_REDIS_REST_URL",
      ),
      token: env(
        "EXPO_PUBLIC_UPSTASH_CACHE_REST_TOKEN",
    ...
    session: {
      url: env(
        "EXPO_PUBLIC_UPSTASH_SESSION_REST_URL",
        "EXPO_PUBLIC_UPSTASH_REDIS_REST_URL",
      ),
      token: env(
        "EXPO_PUBLIC_UPSTASH_SESSION_REST_TOKEN",
        "EXPO_PUBLIC_UPSTASH_REDIS_REST_TOKEN",
      ),
      timeout: 10000,
    },
