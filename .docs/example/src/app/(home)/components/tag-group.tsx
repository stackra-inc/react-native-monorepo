...
const CategoriesTagGroup: React.FC<CategoriesTagGroupProps> = ({
  ...
  const getColorClassName = (isSelected: boolean) => {
    return cn(isSelected ? "accent-accent-soft-foreground" : "accent-field-foreground");
  };
    ...
    >
      <TagGroup.List className="justify-center">
        <TagGroup.Item id="news">
          {({ isSelected }) => (
            <>
              <SquareArticleIcon size={16} colorClassName={getColorClassName(isSelected)} />
        ...
        <TagGroup.Item id="travel">
          {({ isSelected }) => (
            <>
              <GlobeIcon size={14} colorClassName={getColorClassName(isSelected)} />
        ...
        <TagGroup.Item id="gaming">
          {({ isSelected }) => (
            <>
              <RocketIcon size={14} colorClassName={getColorClassName(isSelected)} />
...
const SingleSelectionContent = () => {
  const [selected, setSelected] = useState<Set<string | number>>(new Set(["news"]));
...
const MultipleSelectionContent = () => {
  const [selected, setSelected] = useState<Set<string | number>>(new Set(["news", "travel"]));
  ...
  return (
    <View className="flex-1 px-5 items-center justify-center gap-6">
      ...
      <View className="gap-2">
        ...
        <TagGroup selectionMode="single" disabledKeys={new Set(["travel", "gaming"])}>
          <TagGroup.List>
            <TagGroup.Item id="news">News</TagGroup.Item>
            <TagGroup.Item id="travel">Travel</TagGroup.Item>
            <TagGroup.Item id="gaming">Gaming</TagGroup.Item>
  ...
  return (
    <View className="flex-1 px-5 items-center justify-center">
      ...
      >
        ...
        <Description hideOnInvalid>{`Selected: ${Array.from(selected).join(", ")}`}</Description>
...
const WithRemoveButtonFullContent = () => {
  ...
  const onRemoveFrameworks = (keys: Set<string | number>) => {
    setFrameworks((prev) => prev.filter((framework) => !keys.has(framework.id)));
  };
  ...
  return (
    <View className="flex-1 px-5 justify-center gap-8">
      <TagGroup selectionMode="single" onRemove={onRemoveTags}>
        ...
        <TagGroup.List
          renderEmptyState={() => (
            <AppText className="text-sm text-muted p-1">No categories found</AppText>
          )}
        >
          {tags.map((tag) => (
            <AnimatedTagGroupItem key={tag.id} id={tag.id} layout={LinearTransition.springify()}>
      ...
      <TagGroup selectionMode="single" onRemove={onRemoveFrameworks}>
        ...
        <TagGroup.List
          renderEmptyState={() => (
            <AppText className="text-sm text-muted p-1">No frameworks found</AppText>
          )}
        >
          {frameworks.map((fw) => (
            <AnimatedTagGroupItem key={fw.id} id={fw.id} layout={LinearTransition.springify()}>
              {({ isSelected }) => (
                <>
                  <TagGroup.ItemLabel>{fw.name}</TagGroup.ItemLabel>
                  <TagGroup.ItemRemoveButton
                    className={cn("p-0.5 bg-muted", isSelected && "bg-accent-soft-foreground")}
                    iconProps={{
                      color: isSelected ? themeColorAccentForeground : themeColorBackground,
                    }}
                  />
                </>
              )}
  ...
  return (
    <View className="flex-1 px-5 justify-center gap-8">
      ...
      >
        ...
        <TagGroup.List
          renderEmptyState={() => (
            <AppText className="text-sm text-muted p-1">No team members</AppText>
          )}
        >
          {users.map((user) => (
            <AnimatedTagGroupItem
      ...
      {Array.from(selectedKeys).length > 0 && (
        <View className="gap-2">
          <AppText className="text-sm font-medium text-muted">Selected:</AppText>
